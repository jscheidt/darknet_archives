...
for l in lines
	...
	split = l.gsub(/"/, '').strip.split(/\t|<=0=>|<====>|<===>|<==>|<=A=>|<F=L>/)
	cosine = split[1]
	market1 = split[2]
	user1 = l.gsub(/"/, '').strip.split(/\t|<=0=>|<====>/)[2]
	activity1 = split[4]
	firstDiff1 = split[5]
	firstPost1 = marketDates[market1].split(":")[0].to_i + firstDiff1.to_i
	lastDiff1 = split[6]
	lastPost1 = marketDates[market1].split(":")[1].to_i - lastDiff1.to_i
	market2 = split[7]
	user2 = l.gsub(/"/, '').strip.split(/\t|<=0=>|<====>/)[3]
	activity2 = split[9]
	firstDiff2 = split[10]
	firstPost2 = marketDates[market2].split(":")[0].to_i + firstDiff2.to_i
	lastDiff2 = split[11]
	lastPost2 = marketDates[market2].split(":")[1].to_i - lastDiff2.to_i
